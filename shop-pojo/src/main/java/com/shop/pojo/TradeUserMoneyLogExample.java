...
public class TradeUserMoneyLogExample implements Serializable {
    ...
    public TradeUserMoneyLogExample() {
        //实例化TradeUserMoneyLogExanple对象时，初始化封装Criteria对象的集合
    ...
    public Criteria createCriteria() {
        Criteria criteria = createCriteriaInternal();
        //判断Criteria集合中是否存在Criteria对象
        if (oredCriteria.size() == 0) {
            //不存在，这放入对象
            oredCriteria.add(criteria);
        }
        //存在则返回Criteria对象并不是集合对象，疑问：Criteria集合中只有一个Criteria对象？
        return criteria;
    }
    ...
    protected abstract static class GeneratedCriteria {
        ...
        //初始化Criteria实例时，会调用这个方法,创建Criterion的集合
